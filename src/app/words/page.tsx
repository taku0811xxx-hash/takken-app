...
function WordsContent() {
  ...
  useEffect(() => {
    getCheckedWords().then(data => setChecked(data)).catch(() => {}).finally(() => setLoading(false))
  }, [])
  ...
  const categoryWords = useMemo(() => words.filter(w => w.id.startsWith(activeCategory)), [words, activeCategory])
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pb-24 md:pb-8">
      ...
      <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
        {CATEGORIES.map(cat => (
          <button key={cat.prefix} onClick={() => { setActiveCategory(cat.prefix); setActiveTab('all') }}
            className={`flex-shrink-0 px-3 py-2 rounded-xl text-xs font-medium transition-all flex items-center gap-1 ${activeCategory === cat.prefix ? 'bg-gray-800 text-white' : 'bg-white text-gray-500 border border-gray-100'}`}>
      ...
      <div className="flex gap-2 mb-4">
        {([['all', 'すべて', categoryWords.length], ['unchecked', '未チェック', categoryWords.length - checkedCount], ['checked', '理解済み', checkedCount]] as [Tab, string, number][]).map(([tab, label, count]) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`flex-1 py-2 rounded-xl text-xs font-medium transition-all ${activeTab === tab ? 'bg-gray-800 text-white' : 'bg-white text-gray-500 border border-gray-100'}`}>
            {label}({count})
          </button>
        ))}
      </div>
